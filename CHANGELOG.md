...
### Changed
- Saved configs and the HF token now always live on the controller, so they are the same no
  matter which node is armed (they used to be written to whichever worker was armed, and
  disappeared with the node).
